...
func main() {
	...
	for {
		...
		case 3:
			...
			switch pilih {
			case 1:
				urutkanTanggal(&data, nProject)
			case 2:
				urutkanTingkatKesulitan(&data, nProject)
			case 3:
				fmt.Println("Kembali ke menu...")
			default:
				fmt.Println("Pilihan tidak valid!")
...
/*
	Fungsi mencari indeks array berdasarkan ID project menggunakan Sequential Search.
	Mengembalikan indeks jika ditemukan, atau -1 jika tidak ditemukan.
*/
func seqSearch(P tabProject, nP int, targetID string) int {
	for i := 0; i < nP; i++ {
		if P[i].id == targetID {
			return i // Ditemukan, kembalikan indeksnya
		}
	}
	return -1 // Tidak ditemukan
}

// Fungsi dummy tambahan agar case 1 di menu pencarian tidak error
func cariNama() {
	fmt.Println("Fitur cari berdasarkan nama belum diimplementasikan.")
}

/*
	Prosedur mencari project berdasarkan kategori menggunakan Binary Search.
	Data di-sort terlebih dahulu menggunakan Insertion Sort sebelum dicari.
*/
func cariKategori(P tabProject, nP int) {
	if nP == 0 {
		fmt.Println("Data project kosong.")
		return
	}

	// 1. Proses Sorting (Insertion Sort) berdasarkan Kategori secara Ascending
	for i := 1; i < nP; i++ {
		key := P[i]
		j := i - 1
		for j >= 0 && P[j].kategori > key.kategori {
			P[j+1] = P[j]
			j--
		}
		P[j+1] = key
	...
	// 2. Input Kategori yang dicari
	var targetKategori string
	fmt.Print("Masukkan Kategori Project yang dicari: ")
	fmt.Scan(&targetKategori)

	// 3. Proses Binary Search
	low := 0
	high := nP - 1
	foundIdx := -1
	...
	for low <= high {
		mid := (low + high) / 2
		if P[mid].kategori == targetKategori {
			foundIdx = mid
			break // Ketemu
		} else if P[mid].kategori < targetKategori {
			low = mid + 1
		} else {
			high = mid - 1
		}
	...
	// 4. Cetak Hasil
	if foundIdx != -1 {
		fmt.Println("\nProject ditemukan (setelah sorting):")
		fmt.Printf("ID: %s | Nama: %s | Kategori: %s | Kesulitan: %d\n",
			P[foundIdx].id, P[foundIdx].nama, P[foundIdx].kategori, P[foundIdx].kesulitan)
	} else {
		fmt.Println("Project dengan kategori tersebut tidak ditemukan.")
	}
}

/*
	Prosedur mencari project berdasarkan tingkat kesulitan menggunakan Sequential Search.
	Menampilkan semua project yang memenuhi kriteria.
*/
func cariTingkatKesulitan(P tabProject, nP int) {
	if nP == 0 {
		fmt.Println("Data project kosong.")
		return
	}

	var targetKesulitan int
	fmt.Print("Masukkan Tingkat Kesulitan yang dicari (1-5): ")
	fmt.Scan(&targetKesulitan)

	fmt.Println("\nHasil Pencarian Tingkat Kesulitan:")
	ketemu := false
	for i := 0; i < nP; i++ {
		if P[i].kesulitan == targetKesulitan {
			fmt.Printf("- ID: %s | Nama: %s | Kesulitan: %d\n", P[i].id, P[i].nama, P[i].kesulitan)
			ketemu = true
		}
	}

	if !ketemu {
		fmt.Println("Tidak ada project dengan tingkat kesulitan tersebut.")
	}
}

/*
	Prosedur mengurutkan data berdasarkan tanggal menggunakan algoritma Insertion Sort (Ascending).
*/
func urutkanTanggal(P *tabProject, nP int) {
	if nP == 0 {
		fmt.Println("Data project kosong, tidak ada yang bisa diurutkan.")
		return
	}
	...
	for i := 1; i < nP; i++ {
		key := P[i]
		j := i - 1
		// Membandingkan string tanggal format YYYYMMDD
		for j >= 0 && P[j].tanggal > key.tanggal {
			P[j+1] = P[j]
			j--
		}
		P[j+1] = key
	}
	fmt.Println("Project berhasil diurutkan berdasarkan tanggal!")
	tampilkanProject(*P, nP)
}

/*
	Prosedur mengurutkan data berdasarkan tingkat kesulitan menggunakan algoritma Selection Sort (Descending).
*/
func urutkanTingkatKesulitan(P *tabProject, nP int) {
	if nP == 0 {
		fmt.Println("Data project kosong, tidak ada yang bisa diurutkan.")
		return
	}
	...
	for i := 0; i < nP-1; i++ {
		maxIdx := i
		for j := i + 1; j < nP; j++ {
			if P[j].kesulitan > P[maxIdx].kesulitan {
				maxIdx = j
			}
		}
		// Tukar posisi (Swap)
	...
	fmt.Println("Project berhasil diurutkan berdasarkan tingkat kesulitan terbesar!")
	tampilkanProject(*P, nP)
}
